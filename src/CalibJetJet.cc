...
void CalibJetJet::setup(const edm::ParameterSet& pSet, TTree* CalibTree) {

  //tower for jet 1
  const int kMaxTower = 10000;
  towj1et = new float[kMaxTower];
  towj1eta = new float[kMaxTower];
  towj1phi = new float[kMaxTower];
  towj1en = new float[kMaxTower];
  towj1em = new float[kMaxTower];
  towj1hd = new float[kMaxTower];
  towj1oe = new float[kMaxTower];
  towj1id_phi = new int[kMaxTower];
  towj1id_eta = new int[kMaxTower];
  towj1id     = new int[kMaxTower];
  ...
  //tower for jet 1
  towj2et = new float[kMaxTower];
  towj2eta = new float[kMaxTower];
  towj2phi = new float[kMaxTower];
  towj2en = new float[kMaxTower];
  towj2em = new float[kMaxTower];
  towj2hd = new float[kMaxTower];
  towj2oe = new float[kMaxTower];
  towj2id_phi = new int[kMaxTower];
  towj2id_eta = new int[kMaxTower];
  towj2id     = new int[kMaxTower];
  ...
  // Next-toleading Gen-Jet- branches of the tree 
...
/* **Analyze the event** */
void CalibJetJet::analyze(const reco::CaloJetCollection& calojet,
			  const reco::GenJetCollection&  genjet,
  ...
  for(reco::CaloJetCollection::const_iterator jit1 = calojet.begin(); 
      jit1!=calojet.end(); ++jit1){
     if (fabs(jit1->eta()) > eta_cut) continue;
     for (reco::CaloJetCollection::const_iterator jit2 = jit1;
          jit2!=calojet.end(); ++jit2){
        if (fabs(jit2->eta()) > eta_cut) continue;
	for (reco::CaloJetCollection::const_iterator jit3 = jit2;
             jit2!=calojet.end(); ++jit3){
  ...
  for(reco::GenJetCollection::const_iterator jit1 = genjet.begin(); 
      jit1!=genjet.end(); ++jit1){
     if (fabs(jit1->eta()) > eta_cut) continue;
     for (reco::GenJetCollection::const_iterator jit2 = jit1;
          jit2!=genjet.end(); ++jit2){
        if (fabs(jit2->eta()) > eta_cut) continue;
	for (reco::GenJetCollection::const_iterator jit3 = jit2;
             jit2!=genjet.end(); ++jit3){
          if (fabs(jit3->eta()) > eta_cut) continue;
          s_sum_et += jit3->pt();
	}
        if (s_sum_et<15. && s_sum_et/(jit1->pt()+jit2->pt())<0.1 ){
